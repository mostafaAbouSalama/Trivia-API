...
def create_app(test_config=None):
    ...
    def retrieve_questions():
        ...
        for category in query_of_categories:
            categories_dict[category.id] = category.type
        #   Making sure we got questions from database and
        #   after passing the pagination
    ...
    def create_or_search_for_question():
        ...
        #   Capturing the values of the forms to check later
        #   if this is a search or adding a new question
    ...
    def play_quiz():
        body = request.get_json()
        #   Grab the quiz category and list of previously asked questions
        #   if we are in the middle of the quiz
        ...
        #   Make sure we have a unique set of previously asked questions
        #   where no question is repeated
        prev_questions_uniq = set(previous_questions)
        #   Check for the category id to query the respective category
        ...
        #   If so, I add it to a new list quiz_questions which I will use
        #   to feed the quiz itself
        quiz_questions = [question for question in questions if question.id
                          not in prev_questions_uniq]
        if len(quiz_questions) > 0:
            #   Check we have questions in our newly made quiz_questions list
            #   If so, I pick one question from it randomly
            the_question = quiz_questions[random.randint(0,
                                          len(quiz_questions) - 1)]
            return jsonify({
                'success': True,
